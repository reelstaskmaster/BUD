...
const result = db.prepare("UPDATE settings SET value=? WHERE key = \"unified_api_key\"").run(key);
if (result.changes !== 1) throw new Error("unified_api_key setting was not updated");
const check = db.prepare("SELECT value FROM settings WHERE key = \"unified_api_key\"").get();
db.close();
